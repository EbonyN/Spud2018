...
switch(goal){                                            //a switch is used so that only one program is needed for each diferent scenario of goals
 ...
 default:
   ...
   if((ballX == -1 || ballX == 0) && ballClose != -1){
          ballX = previousBallX;
     if(ballX>130 && ballX<230){
    Serial.println("ball middle  ");
          if(goalX>80 && goalX<280){
            base->movePlayerForward(base->MAX_SPEED);
           Serial.print("  goal seen  "); Serial.println(goalX); 
          }
          if(goalX<81 && goalX != -1){
             Serial.println("goal to left");
           for(int a=0; a<35; a++){
      ...
      if(ballClose == 1){
        Serial.print("ball close  "); Serial.println(ballX);
        if(ballX<101 && ballX != -1){
          Serial.println("ball close, left  ");
          base->movePlayerLeft(base->MAX_SPEED);
        }
        if(ballX>259){
          Serial.println("ball close, right  ");
          base->movePlayerRight(base->MAX_SPEED);
        }
        if(ballX>100 && ballX<260 && ballY !=-1){
          Serial.print("  ball close-middle  "); Serial.println(goalX);
          if(goalX>80 && goalX<280){
            base->movePlayerForward(base->MAX_SPEED);
           Serial.print("  goal seen  "); Serial.println(goalX); 
          }
      ...
      Serial.print("ball not close  "); Serial.println(ballX);
      if(ballX >229){
        Serial.print("right  "); Serial.println(ballX);
        base->rotatePlayerRight(base->MAX_SPEED);
      }
      if(ballX <131 && ballX != -1){
        Serial.print("left  "); Serial.println(ballX);
        base->rotatePlayerLeft(base->MAX_SPEED);
      }
      }
  }
